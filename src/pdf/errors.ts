...
export class PDFError extends Error {
  static serialize(error: unknown, depth = 0): SerializedError {
    if (depth > 4) {
    ...
    if (error instanceof Error) {
      ...
      if (withFields.cause) {
        serialized.cause = PDFError.serialize(withFields.cause, depth + 1);
      }
      return serialized;
    }
...
export class PdfFontSourceFetchError extends PDFError {
...
export class PdfFontRegistrationError extends PDFError {
