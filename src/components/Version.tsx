 ...
 */
export function transformVersion(version: string) {
  ...
  if (count === "0") {
    return dirty ? `v${base}.dirty` : `v${base}`;
  }
  return `v${base}dev${count}+${hash}${dirty ? ".dirty" : ""}`;
}

export function Version({ value }: { value: string }) {
  const version = useMemo(() => transformVersion(value), [value]);
  if (!version.trim()) {
    return null;
  }

  return <span className="text-[0.6875rem] leading-none text-text-placeholder">{version}</span>;
}
